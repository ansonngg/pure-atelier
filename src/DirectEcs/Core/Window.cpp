...
void Window::Resize(uint32_t width, uint32_t height)
{
    ...
    ThrowIfFailed(dxgiSwapChain_->GetDesc(&swapChainDesc));
    ThrowIfFailed(
        dxgiSwapChain_->ResizeBuffers(
            BufferCount,
            clientWidth_,
            clientHeight_,
...
void Window::CreateSwapChain_(bool isTearingSupported)
{
    ...
    ThrowIfFailed(factory->MakeWindowAssociation(windowHandle_, DXGI_MWA_NO_ALT_ENTER));

    ThrowIfFailed(swapChain1.As(&dxgiSwapChain_));
    currentBackBufferIndex_ = dxgiSwapChain_->GetCurrentBackBufferIndex();
}
...
void Window::CreateDescriptorHeap_()
{
    ...
    ThrowIfFailed(device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&d3d12RTVDescriptorHeap_)));
...
void Window::UpdateRenderTargetViews_()
{
    ...
    for (int i = 0; i < BufferCount; ++i)
    {
        ...
        ThrowIfFailed(dxgiSwapChain_->GetBuffer(i, IID_PPV_ARGS(&backBuffer)));
...
{
    ThrowIfFailed(dxgiSwapChain_->Present(syncInterval_, presentFlags_));
}
